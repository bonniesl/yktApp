<template>
    <div class="app">
        ...
        <div class="map">
           <Map class="mapc" :tracks="runoverList"></Map>
                    ...
                    <div class="disleft">
                        <div class="distttop">
                          ...
                          <div class="disup">
                              ...
                              <text class="disnum1">{{runoverList.valid_metre ? (runoverList.valid_metre/1000).toFixed(2) : '0'}}</text>
                          </div>
                        </div>
                        
                        <text class="distimes">{{runoverList.c_time}}</text>
                        
                    </div>
                    <div class="disright">
                        <image class="avator" :src="defaultAvator" style="width:120px;height:120px;border-radius:120px;"></image>
                        <text class="disrtext">{{userInfos=='' ? userInfo.name : userInfos.name }}</text>
                    </div>
                </div>
                    ...
                    <div class="speedTimebar1"  v-for="(item,index) in metre_list" :key="index">
                      ...
                      <div class="sbar" v-if="index<(len-1)">
                          ...
                          <div class="bart">
                            ...
                            <text class="innerbartext">{{s_to_hs(item.spend_time)}}</text>
                          </div>
                      </div>
                      <text class="donwtext" v-if="index<(len-1) && item.metre>1000 && (index+1)%5==0">{{index+1}}公里 累计用时{{fiveUTime}}</text>
                      <!--text class="donwtext" v-if="(index+1)==len">{{index+1}}公里 累计用时{{sallTotal}}</text-->
                      <text class="donwtext" v-if="index==(len-1) && item.metre<1000">最后不足1公里 累计用时{{notFiveTime}}</text>
                   </div>
                </div>
                 <text class="meterMore" @click="moreMClick" v-if="runoverList.run_type==1">查看详情</text>
            </div>
...
import Map from "../../components/mapPosition/index";
      ...
      getUserRunInfo(){
         ...
         stream.fetch({
                  ...
                  if(res.status == '200'){
                    ...
                    self.runRracks=res.data.tracks_list; 
                    ...
                    let userRunInfo =res.data.course_class_user_run_infos;
                    ...
                    if(metre_list.length!=0){
                        let curPage =Math.ceil(metre_list.length / 10);
                    ...
                    self.notFiveTime=self.formatSeconds(x);
                  ...
                  if(res.status == 200){
                      ...
                      if(this.userInfos==''){
                        if(this.userInfo.avatar_file == ''){
                          ...
                          this.defaultAvator = this.userInfos.avatar_file;
                      }
